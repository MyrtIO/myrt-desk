...
struct HSV {
  ...
  constexpr HSV(uint8_t ih, uint8_t is, uint8_t iv) __attribute__((always_inline)) : h(ih), s(is), v(iv) {}
...
void fill_gradient(
  RGB* targetArray,
  uint16_t startpos, HSV startcolor,
  uint16_t endpos,
  HSV endcolor,
  TGradientDirectionCode directionCode  = SHORTEST_HUES );


void fill_gradient(RGB* targetArray, uint16_t numLeds, const HSV& c1, const HSV& c2,
					TGradientDirectionCode directionCode = SHORTEST_HUES );

void fill_gradient(RGB* targetArray, uint16_t numLeds,
					const HSV& c1, const HSV& c2, const HSV& c3,
					TGradientDirectionCode directionCode = SHORTEST_HUES );


void fill_gradient(RGB* targetArray, uint16_t numLeds,
					const HSV& c1, const HSV& c2, const HSV& c3, const HSV& c4,
					TGradientDirectionCode directionCode = SHORTEST_HUES );
